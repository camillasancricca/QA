...
def query_table_multiple(table_csv, questions):
    ...
    Give each answer on a new line with the corresponding number, in order. Do not add explanations or repeat the question.
...
def evaluate_table_variants(table_csv, question_list, r_answer_list, df_variants, variant_name, acc_stats):
    ...
    for i, df in enumerate(df_variants):
        ...
        gen_answers = query_table_multiple(csv_version, question_list)
...
for group_id, indexes in enumerate(normalized_map.values()):
    ...
    if (group_id >= 0):
        ...
        # CLEAN
        clean_answers = query_table_multiple(table_csv, question_list)
        ...
        for variant_name, dirty_func in [
            ...
            for seed_variant in range(3):
                ...
                save += evaluate_table_variants(table_csv, question_list, r_answer_list, dirty_versions, variant_name, accuracy_stats)
